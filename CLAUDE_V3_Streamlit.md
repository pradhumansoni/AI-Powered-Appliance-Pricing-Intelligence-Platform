...
# │   │   └── ac\_cleaned.csv      # 994 rows, 20 columns (already done)
...
# │       └── appliances\_master.csv

# │

# ├── notebooks/

# │   ├── 01\_eda\_ac.ipynb         # EDA for AC dataset

# │   ├── 02\_feature\_engineering.ipynb

# │   ├── 03\_model\_experiments.ipynb

# │   └── 04\_shap\_explainability.ipynb
...
# │   └── ac\_xgboost\_v1.pkl       # Versioned saved models
...
# │   └── test\_preprocess.py      # At minimum: test schema, dtypes, no nulls
...
# | product\_name | object | Drop for modeling, keep for display |
...
# | star\_rating | int64 | Energy efficiency stars — keep |

# | ac\_type | object | 89.4% Split — \*\*DROP\*\* |
...
# | Wi-Fi Connectivity | int64 | Merge into smart\_features |

# | APP Control | int64 | Merge into smart\_features |

# | Voice Control | int64 | r = 0.17 — Merge into smart\_features |
...
# capacity, star\_rating, inverter, brand, Self Diagnosis,

# Hidden Panel Display, Auto Clean, Air Swing, PM 2.5 Filter,

# 4-Way Air Swing, smart\_features (engineered), inverter\_x\_star (engineered)
...
# product\_name (display only), rating (r = -0.01), ac\_type (89% imbalanced),
...
# df\['smart\_features'] = df\['Wi-Fi Connectivity'] + df\['APP Control'] + df\['Voice Control']

# 

# \# 2. Interaction term: inverter × star\_rating captures premium tier

# df\['inverter\_x\_star'] = df\['inverter'] \* df\['star\_rating']
...
# \### Stage 1 — EDA (notebook: 01\_eda\_ac.ipynb)
...
# \- smart\_features distribution (bar chart of 0/1/2/3 counts)
...
# def preprocess\_ac(df: pd.DataFrame) -> pd.DataFrame:

# &#x20;   # 1. Drop low-signal columns

# &#x20;   # 2. Engineer smart\_features and inverter\_x\_star
...
# \### Stage 3 — Model Experiments (notebook: 03\_model\_experiments.ipynb)
...
# cv = KFold(n\_splits=5, shuffle=True, random\_state=42)
...
# \### Stage 4 — SHAP Explainability (notebook: 04\_shap\_explainability.ipynb)
...
# explainer = shap.TreeExplainer(best\_model)
...
# \# 1. shap.summary\_plot() — global feature importance

# \# 2. shap.waterfall\_plot() — single prediction explanation

# \# 3. shap.dependence\_plot('capacity') — how capacity drives price
...
# \*\*Gate:\*\* SHAP values must sum (approximately) to prediction − base\_value. If they don't, something is wrong in the explainer setup.
...
# def predict\_price(input\_dict: dict, category: str = "ac") -> dict:

# &#x20;   """

# &#x20;   Returns:

# &#x20;   - predicted\_price: float

# &#x20;   - shap\_explanation: dict of feature -> contribution

# &#x20;   - alternatives: list of products within ±15% predicted price

# &#x20;   """

# &#x20;   model = joblib.load(f"models/{category}\_xgboost\_v1.pkl")

# &#x20;   # ... preprocess input\_dict using category-specific preprocessor
...
# \### Stage 6 — Streamlit App (app/streamlit\_app.py)
...
# \- Cache model loading with `@st.cache\_resource`

# \- Cache data loading with `@st.cache\_data`

# \- Use `st.columns()` for the main layout — predicted price on left, SHAP chart on right

# \- No `st.experimental\_rerun()` — use session state properly
...
# product\_name, brand, category, price, rating, review\_count,

# energy\_rating, capacity\_value, capacity\_unit, warranty\_years
...
# \- Solution: `capacity\_value` (float) + `capacity\_unit` (string) as separate columns, OR normalize within-category to a percentile rank

# 

# \*\*Appliance-specific columns:\*\*

# \- Add sparse columns (e.g., `ac\_inverter`, `wm\_spin\_speed`, `fridge\_doors`)
...
# &#x20;   "ac": "models/ac\_xgboost\_v1.pkl",

# &#x20;   "washing\_machine": "models/wm\_xgboost\_v1.pkl",
...
# | SHAP fails on a product | Fallback to feature importance from model.feature\_importances\_ |

# | Model R² < 0.80 | Do not deploy. Go back to feature engineering. Log which features are hurting performance. |

# | Capacity = 0.75 (window AC) | Investigate — if genuine, keep with a `is\_window\_ac` flag. If scrape error, drop. |
...
# \- \[ ] Cross-validation uses `random\_state=42` consistently
...
# \- \[ ] Streamlit runs locally with `streamlit run app/streamlit\_app.py`
